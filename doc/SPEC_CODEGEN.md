...
The same three-bucket model applies to **both backend and frontend**:

| Bucket | Who owns it | Blast behaviour |
|--------|-------------|-----------------|
| `generated/` | Blast | Rewritten wholesale on `blast gen`; never hand-edit |
| `custom/` | User | Never read, touched, deleted, or renamed by Blast |
| Vendored framework files (from `templates/canonical/`) | Shared; user CAN edit | `blast vendor-update` will stomp; fork-by-default means edits persist on local checkout |

### Backend split

**In-layer** subdir convention. Every Rust layer with codegen has both:
