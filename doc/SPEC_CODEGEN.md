...
| `Components` | + leptos forms (`transport/leptos/components/generated/forms/<r>/`). |
...
src/transport/leptos/components/generated/forms/<r>/create_form.rs   leptos-form derive on UserInsertable
src/transport/leptos/components/generated/forms/<r>/edit_form.rs     leptos-form derive on UserPatch
src/transport/leptos/components/generated/forms/<r>/mod.rs

src/transport/leptos/components/generated/tables/<r>.rs              leptos-struct-table derive on UserPublic
src/transport/leptos/components/generated/tables/mod.rs
...
Currently `WATCHED_DIRS` covers `src/{structs,models,flows,transport/http,transport/ws}/generated`. Extension to `src/transport/leptos/.../generated` lands when the leptos emitters do (phase 10 per `docs/wip/leptos_migration.md`).
...
`src/transport/leptos/` is itself the layer's user-owned root. Inside it:

```
src/transport/leptos/
├── mod.rs                       user
├── app.rs                       user — <App> root + <Routes> tree
├── client.rs                    user — wasm hydrate entry
├── api_client.rs                user — wasm-only HTTP wrapper
├── theme.rs / icons.rs          user — thaw theme + icondata registry
├── components/
│   ├── auth_guard.rs            user — hand-written
│   ├── error_banner.rs          user
│   ├── page_shell.rs            user
│   └── generated/
│       ├── forms/<r>/           Blast — leptos-form derives
│       └── tables/<r>.rs        Blast — leptos-struct-table derives
├── pages/
│   ├── welcome.rs / login.rs    user — baseline auth pages, handwritten
│   └── generated/<r>/           Blast — admin-style CRUD pages
├── data/
│   ├── auth.rs                  user — login/register/logout helpers
│   └── generated/<r>.rs         Blast — per-resource isomorphic helpers
├── routes/
│   └── generated.rs             Blast — concatenates all <Route> entries
└── signals/
    ├── toast.rs                 user — global toast singleton
    └── relay.rs                 user — WS subscription primitive
```
