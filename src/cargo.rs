...
async fn search_crates(query: &str) -> Result<Vec<Crate>, Box<dyn Error>> {
    ...
    let response = client.get(&url).header("User-Agent", "blast/0.1.0").send().await?;
...
fn format_crate_choices(crates: &[Crate]) -> Vec<String> {
    crates
        ...
        .map(|c| {
            ...
            format!("{} v{} ({} downloads) - {}", c.name, c.max_version, format_downloads(c.downloads), desc)
        })
        .collect()
}
...
pub async fn add_dependency(config: &Config, search_term: &str) -> Result<(), Box<dyn Error>> {
    ...
    let search = if search_term.is_empty() {
        let search: String = Input::with_theme(&ColorfulTheme::default()).with_prompt("Enter search term").interact_text()?;
        search
    } else {
    ...
    let selection = FuzzySelect::with_theme(&ColorfulTheme::default()).with_prompt("Select crate").default(0).items(&choices).interact()?;
    ...
    match add_dependency_to_cargo_toml(config, &selected_crate.name, &version) {
        Ok(_) => {
            ...
            match std::process::Command::new("cargo").args(["update", "-p", &selected_crate.name]).current_dir(&config.project_dir).output() {
                Ok(_) => {
...
pub fn remove_dependency(config: &Config) -> Result<(), Box<dyn Error>> {
    ...
    let dep_choices: Vec<String> = dep_names
        .iter()
        .map(|name| {
    ...
    let selections = MultiSelect::with_theme(&ColorfulTheme::default()).with_prompt("Select dependencies to remove").items(&dep_choices).interact()?;
    ...
    let selected_deps: Vec<String> = selections.iter().map(|&i| dep_names[i].clone()).collect();
    ...
    let proceed = dialoguer::Confirm::with_theme(&ColorfulTheme::default()).with_prompt("Proceed with removal?").default(false).interact()?;
