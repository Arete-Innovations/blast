...
fn render_top_barrel(tables: &[&str]) -> String {
    ...
    sorted.sort();
    let mut out = String::new();
    out.push_str("//! Per-resource flow subdirectories. Generated by Blast.\n\n");
    for t in sorted {
...
fn verb_signature(verb: Verb, type_stem: &str) -> (String, String, String) {
    match verb {
        ...
        Verb::Create => (
            ...
            String::from("create::run(ctx, input.clone())"),
        ),
        Verb::Update => (
            format!(", id: i64, patch: {type_stem}Patch"),
            format!("{type_stem}Public"),
            String::from("update::run(ctx, id, patch.clone())"),
        ),
...
mod tests {
    ...
    fn emits_barrel_and_verb_files_with_headers() {
        ...
        assert!(list_body.contains("Crank::none().run(|| routines::generated::users::list::run(ctx, query.clone()))"), "list flow must wrap routine in Crank: {}", list_body);
        ...
        assert!(update_body.contains("ctx.require_any(&[Role::Admin, Role::Editor])?;"), "Roles should emit sorted Role enum variants, got: {}", update_body);
