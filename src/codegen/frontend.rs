...
fn emit_validators(project_root: &Path, resources: &[ResourceIr]) -> BlastResult<()> {
    ...
    for r in resources {
        let mut body = header::marker_for_resource(project_root, &r.table)?;
...
fn emit_per_resource_list_helpers(
    ...
    for r in resources {
        ...
        let marker = header::marker_for_resource(project_root, &r.table)?;
...
fn build_resource_list_ts(verb: &ir_loader::VerbSpec) -> String {
