...
pub fn launch_manager(config: &mut Config) {
    ...
    loop {
        ...
        let env_status = if config.environment == "dev" {
            style("dev").green()
        } else {
            style("prod").red()
        };
        ...
        let warnings_status = if config.show_compiler_warnings {
            style("enabled").green()
        } else {
        ...
        let git_status = if toml_value.get("git").is_some() {
            let username = toml_value.get("git")
...
fn edit_git_config(config: &mut Config) {
    ...
    let mut current_remote = toml_value
        .get("git")
    ...
    let mut current_username = toml_value
        .get("git")
        .and_then(|g| g.get("username"))
        .and_then(|u| u.as_str())
        .unwrap_or("")
    ...
    let mut current_email = toml_value
        .get("git")
    ...
    loop {
        ...
        match selection {
            0 => { // Remote URL
                let new_remote: String = Input::new()
                    .with_prompt("Remote repository URL")
                    .default(current_remote.clone())
                    .interact_text()
                    .unwrap();
                
                // Update local variable for display
                if new_remote != current_remote {
                    toml_value["git"].as_table_mut().unwrap()
                        .insert("remote_url".to_string(), toml::Value::String(new_remote.clone()));
            ...
            1 => { // Username
                let new_username: String = Input::new()
                    ...
                    .default(current_username.clone())
                    .interact_text()
                    .unwrap();
                ...
                if new_username != current_username {
                    toml_value["git"].as_table_mut().unwrap()
                        .insert("username".to_string(), toml::Value::String(new_username.clone()));
            ...
            2 => { // Email
                ...
                if new_email != current_email {
                    toml_value["git"].as_table_mut().unwrap()
                        .insert("email".to_string(), toml::Value::String(new_email.clone()));
            ...
            3 => { // Apply to Git Config
                ...
                if !current_email.trim().is_empty() {
                    match std::process::Command::new("git")
                        .args(["config", "user.email", &current_email])
                        .output() {
                ...
                if !current_remote.trim().is_empty() {
                    ...
                    if remotes.contains("origin") {
                        ...
                        match std::process::Command::new("git")
                            .args(["remote", "set-url", "origin", &current_remote])
                            .output() {
                        ...
                        // Add new origin
                        match std::process::Command::new("git")
                            .args(["remote", "add", "origin", &current_remote])
                            .output() {
...
fn edit_project_name(config: &mut Config) {
    ...
    let change_name = Confirm::new()
        .with_prompt(style("Do you want to continue?").bold().to_string())
        .default(false)
        .interact()
        .unwrap();
        ...
        Ok(_) => {
            match fs::write(&catalyst_path, formatted_toml) {
                Ok(_) => {
                    ...
                    println!("{} Project name changed from {} to {}", 
                        style("✅").green(),
                        style(old_name).dim(),
...
fn edit_custom_setting(config: &Config) {
    ...
    let section_selection = Select::new()
        .with_prompt("Select section or create new")
        .items(&all_options)
        .default(0)
        .interact()
        .unwrap();
    ...
    let section_name = if section_selection == sections.len() {
        ...
        toml_value
            .as_table_mut()
            .unwrap()
            .insert(new_section.clone(), toml::Value::Table(toml::value::Table::new()));
    ...
    let value_type_selection = Select::new()
    ...
    let value = match value_types[value_type_selection] {
        ...
        "Boolean" => {
            ...
            let selection = Select::new()
                .with_prompt("Select boolean value")
                .items(&options)
                .default(0)
                .interact()
                .unwrap();
...
pub fn toggle_environment(config: &mut Config) -> Result<(), Box<dyn std::error::Error>> {
    ...
    parsed_toml.as_table_mut().unwrap()
    ...
    let new_env_style = if config.environment == "dev" { style(&config.environment).green() } else { style(&config.environment).red() };
...
pub fn get_project_info_with_paths<P: AsRef<std::path::Path>>(config_path: &str, project_dir: P) -> Result<Config, Box<dyn std::error::Error>> {
    ...
    let show_compiler_warnings = config_val.get("settings")
        .and_then(|s| s.get("show_compiler_warnings"))
