...
pub fn initialize_vessel_database() -> bool {
    ...
    for line in env_content.lines() {
        if line.starts_with("VESSEL_DATABASE_URL=") {
            let url = line["VESSEL_DATABASE_URL=".len()..].trim().trim_matches('"');
            vessel_database_url = Some(url);
            break;
        }
    }
...
pub fn refresh_vessel_database() -> bool {
    ...
    match rollback_output {
        Ok(output) => {
            if output.status.success() {
                progress.success("Rolled back all Vessel migrations");
            } else {
                let error = String::from_utf8_lossy(&output.stderr);
...
// Helper function to generate vessel schema
