    ...
    match output {
        Ok(result) => {
            ...
            if stderr.contains("set the upstream") {
                ...
                let set_upstream = Confirm::new()
                    .with_prompt(format!("Do you want to set upstream for branch '{}'?", branch))
                    .default(true)
                    .interact()
                    .unwrap();
                ...
                if set_upstream {
                    let upstream_output = Command::new("git")
                        ...
                        .output()
                        .expect("Failed to push with upstream");
...
pub fn git_commit() {
    ...
    let add_all = Confirm::new()
        .with_prompt("Do you want to add all files to the commit?")
        .default(false)
        .interact()
        .unwrap();
    ...
    if add_all {
        ...
        let files_list = String::from_utf8_lossy(&files_output.stdout)
            .lines()
            .map(|s| s.to_string())
            .collect::<Vec<String>>();
        ...
        for &idx in &selections {
            ...
            let add_output = Command::new("git")
                .args(["add", file])
                .output()
                .expect(&format!("Failed to add file: {}", file));
                
            if !add_output.stderr.is_empty() {
                println!("⚠️ Error adding file {}: {}", 
                    file, 
    ...
    match options[selection] {
        "Set Username" => {
            let username: String = Input::new()
                .with_prompt("Enter Git username")
                .interact_text()
                .unwrap();
        ...
        "Set Remote Origin URL" => {
            let url: String = Input::new()
                .with_prompt("Enter remote origin URL")
                .interact_text()
                .unwrap();
            ...
            if !url.trim().is_empty() {
                ...
                let remote_check = Command::new("git")
                    .args(["remote"])
                    .output()
                    .expect("Failed to check remotes");
