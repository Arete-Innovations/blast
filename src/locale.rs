...
use std::fs::{self, read_to_string, OpenOptions, create_dir_all};
...
pub fn edit_key() {
    ...
    let selection = Select::with_theme(&theme)
        .with_prompt("Locale Management")
        .items(&options)
        .default(0)
        .interact()
        .unwrap_or(5); // Default to Back if error
...
fn view_language_files_interactive(languages: &[String]) {
    ...
    let theme = ColorfulTheme::default();
    let selection = Select::with_theme(&theme)
        .with_prompt("Select a language to view")
        .default(0)
        .items(languages)
        .interact()
        .unwrap_or(0);
...
fn edit_language_key_interactive(languages: &[String]) {
    ...
    let lang_selection = Select::with_theme(&theme)
        .with_prompt("Select a language to edit")
        .default(0)
        .items(languages)
        .interact()
        .unwrap_or(0);
    ...
    let key_selection = FuzzySelect::with_theme(&theme)
        .with_prompt("\nSelect a key to edit")
        .default(0)
        .items(&paths)
        .interact()
...
fn add_language_key_interactive(languages: &[String]) {
    ...
    // Select a section
    let theme = ColorfulTheme::default();
    let section_selection = Select::with_theme(&theme)
        .with_prompt("Select a section for the new key")
        .default(0)
        .items(&sections)
        .interact();
    ...
    // Input the key name
    let key_name_result = Input::<String>::with_theme(&theme)
        .with_prompt("Enter new key name")
        .interact();
    ...
    // Input the key value
    let key_value_result = Input::<String>::with_theme(&theme)
    ...
    for language in languages {
        ...
        let value = if language == default_lang {
            key_value.clone()
        } else {
            TBI_PLACEHOLDER.to_string()
        };
...
fn add_page_interactive(languages: &[String]) {
    ...
    // Input the page path
    let theme = ColorfulTheme::default();
    let page_path_result = Input::<String>::with_theme(&theme)
        .with_prompt("Enter the page path (e.g., 'user/profile' or 'admin/dashboard')")
        .interact();
    ...
    let title_result = Input::<String>::with_theme(&theme)
        .with_prompt(&format!("Enter title for '{}' page", page_path))
        .interact();
    ...
    let subtitle_result = Input::<String>::with_theme(&theme)
        .with_prompt(&format!("Enter subtitle for '{}' page", page_path))
        .interact();
...
fn add_language_interactive(languages: &[String]) -> Option<String> {
    ...
    let theme = ColorfulTheme::default();
    let lang_code_result = Input::<String>::with_theme(&theme)
        .with_prompt("Enter the new language code (e.g., 'fr', 'es', 'de')")
        .interact();
        ...
        if let Some(stdout) = stdout_opt {
            ...
            match current_view {
                View::Overview => {
                    ...
                    write!(stdout, "{}{}{}== BLAST LOCALE MANAGER =={}",
                        cursor::Goto(1, 1),
                        color::Fg(color::Blue),
                        style::Bold,
                        style::Reset
                    ...
                    for (i, lang) in languages.iter().enumerate() {
                        ...
                        write!(stdout, "{}{} {}{}", 
                            ...
                            if i == selected_language { "\x1b[32m" } else { "\x1b[37m" },
                            marker,
                            lang
                        ).unwrap();
                    }
                    ...
                    write!(stdout, "{}{}Controls:{}", 
                        cursor::Goto(1, 5 + languages.len() as u16 + 2),
                        color::Fg(color::Yellow),
                        style::Reset
                    ).unwrap();
                    
                    write!(stdout, "{} ↑/↓: Select language", 
                        cursor::Goto(1, 5 + languages.len() as u16 + 4)
                    ).unwrap();
                    
                    write!(stdout, "{} Enter: View language details", 
                        cursor::Goto(1, 5 + languages.len() as u16 + 5)
                    ).unwrap();
                    
                    write!(stdout, "{} a: Add new language", 
                        cursor::Goto(1, 5 + languages.len() as u16 + 6)
                    ).unwrap();
                    
                    write!(stdout, "{} Esc: Exit", 
                        cursor::Goto(1, 5 + languages.len() as u16 + 7)
                    ).unwrap();
                ...
                View::LanguageDetails => {
                    ...
                    if !languages.is_empty() && selected_language < languages.len() {
                        ...
                        for (section, nodes) in &json_tree {
                            ...
                            for (i, node) in nodes.iter().enumerate() {
                                ...
                                write!(stdout, "{}{}{}{} {}: {}{}",
                                    ...
                                    marker,
                                    node.key,
                                    if node.value.len() > 30 { 
                                        format!("{}...", &node.value[..30])
                                    } else {
                                        node.value.clone()
                                    },
                                    style::Reset
                        ...
                        write!(stdout, "{} ↑/↓: Navigate entries", 
                            cursor::Goto(1, y + 3)
                        ).unwrap();
                        
                        write!(stdout, "{} Enter: Edit selected key", 
                            cursor::Goto(1, y + 4)
                        ).unwrap();
                        
                        write!(stdout, "{} a: Add new key", 
                            cursor::Goto(1, y + 5)
                        ).unwrap();
                        
                        write!(stdout, "{} p: Add new page", 
                            cursor::Goto(1, y + 6)
                        ).unwrap();
                        
                        write!(stdout, "{} Backspace: Return to overview", 
                            cursor::Goto(1, y + 7)
                        ).unwrap();
                    }
                },
            ...
            write!(stdout, "{}{}{}{}Blast Locale Manager | ESC to exit | {} language(s){}",
...
fn get_locale_dir(config: &Option<&Config>) -> String {
    if let Some(conf) = config {
        conf.assets
            .get("locale")
            .and_then(|locale| locale.get("dir"))
            .and_then(|v| v.as_str())
            .unwrap_or(DEFAULT_LOCALE_DIR)
            .to_string()
    } else {
...
fn get_default_language(config: &Option<&Config>) -> String {
    if let Some(conf) = config {
        conf.assets
            .get("locale")
            .and_then(|locale| locale.get("default_language"))
            .and_then(|v| v.as_str())
            .unwrap_or("en")
            .to_string()
    } else {
...
fn edit_existing_key() {
    ...
    let key_selection = FuzzySelect::with_theme(&theme)
        .with_prompt("Select a key to edit")
        .items(&paths)
        .default(0)
        .interact()
        .unwrap_or(0);
...
fn add_new_key() {
    ...
    for lang in get_language_files() {
        let file_path = format!("{}/{}.json", get_locale_dir(&None), lang);
        let value = if lang == default_lang {
            key_value.clone()
        } else {
            TBI_PLACEHOLDER.to_string()
        };
...
fn add_new_page() {
    ...
    // Input the page path
    let page_path: String = Input::new()
        .with_prompt("Enter the page path (e.g., 'user/profile' or 'admin/dashboard')")
        .interact_text()
        .unwrap_or_default();
...
fn add_new_language() {
    ...
    // Input the new language code
    let lang_code: String = Input::new()
        .with_prompt("Enter the new language code (e.g., 'fr', 'es', 'de')")
        .interact_text()
        .unwrap_or_default();
    ...
    match value {
        ...
        Value::Array(arr) => {
            let new_arr = arr.iter()
                .map(|v| replace_with_placeholders(v))
                .collect();
            Value::Array(new_arr)
        },
