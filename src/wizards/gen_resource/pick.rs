...
pub fn select_table<'a>(
    tables: &'a [ParsedTable],
    preselected: Option<&str>,
) -> BlastResult<&'a ParsedTable> {
    match preselected {
...
fn prompt_table(tables: &[ParsedTable]) -> BlastResult<&ParsedTable> {
    ...
    let labels: Vec<String> = tables
        .iter()
        .map(|t| format!("{} ({} cols)", t.name, t.columns.len()))
        .collect();
    let idx = FuzzySelect::with_theme(&theme)
        .with_prompt("Pick a table to author/edit a resource for")
        .items(&labels)
        .default(0)
        .interact()?;
    ...
    match picked {
        ...
        None => Err(BlastError::Invalid(format!(
            "FuzzySelect returned out-of-range index {idx}"
        ))),
    }
}
