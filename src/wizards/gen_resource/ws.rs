...
use crate::state::names::{AuthScopeField, FieldName};
use crate::state::resource::{PayloadShape, ResourceState, TopicScope, WsEventsState};
use dialoguer::{theme::ColorfulTheme, Confirm, FuzzySelect, MultiSelect};
use std::collections::BTreeSet;

const PAYLOAD_LABELS: &[(&str, PayloadShape)] = &[
    ("FullPublicRow", PayloadShape::Public),
    ("Admin", PayloadShape::Admin),
    ("IdOnly", PayloadShape::IdOnly),
];
...
pub fn collect_ws_events(table: &ParsedTable, resource: &mut ResourceState) -> BlastResult<()> {
    ...
    let enable = Confirm::with_theme(&theme)
        .with_prompt("Emit WebSocket events for this resource?")
        .default(prev_enabled)
        .interact()?;
    ...
    let pre_triggers: Vec<bool> = column_names
        .iter()
        .map(|n| prev_triggers.contains(*n))
        .collect();
    ...
    let payload_idx = FuzzySelect::with_theme(&theme)
        .with_prompt("Payload shape")
        .items(&payload_label_strs)
        .default(prev_payload_idx)
        .interact()?;
    ...
    let payload_shape = match payload_entry {
        ...
        None => {
            return Err(BlastError::Invalid(format!(
                "payload FuzzySelect returned out-of-range index {payload_idx}"
            )))
        }
    };

    let prev_topic_idx = previous_topic_index(prev.as_ref());
    let topic_idx = FuzzySelect::with_theme(&theme)
        .with_prompt("Topic scope")
        .items(TOPIC_LABELS)
        .default(prev_topic_idx)
        .interact()?;
    let topic_scope = match topic_idx {
        ...
        n => {
            return Err(BlastError::Invalid(format!(
                "topic FuzzySelect returned out-of-range index {n}"
            )))
        }
    };
...
fn prompt_scoped_topic(
    theme: &ColorfulTheme,
    column_names: &[&str],
    prev: Option<&WsEventsState>,
) -> BlastResult<TopicScope> {
    if column_names.is_empty() {
        return Err(BlastError::Invalid(
            "no columns available to scope topic to".to_string(),
        ));
    }
    ...
    let idx = FuzzySelect::with_theme(theme)
        .with_prompt("Scope topic to which field?")
        .items(column_names)
        .default(default_idx)
        .interact()?;
    ...
    match chosen {
        ...
        None => Err(BlastError::Invalid(format!(
            "topic-scope FuzzySelect returned out-of-range index {idx}"
        ))),
    }
}
