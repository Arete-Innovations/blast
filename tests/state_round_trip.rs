...
fn sample_app() -> AppState {
    ...
    app.sections.insert("services".to_string(), AppPolicySection::Services(services));
...
fn save_is_byte_stable() {
    ...
    let p1 = dir1.path().join(blast::state::io::RESOURCES_DIR).join("users.ron");
    let p2 = dir2.path().join(blast::state::io::RESOURCES_DIR).join("users.ron");
...
fn save_writes_atomic_no_temp_left_behind() {
    ...
    let res = sample_resource();
    blast::state::io::save_resource(dir.path(), &res).expect("save");
    let entries: Vec<_> = std::fs::read_dir(dir.path().join(blast::state::io::RESOURCES_DIR))
        .expect("read_dir")
        .flatten()
...
fn v2_resource_round_trips_with_all_extensions() {
    ...
    assert_eq!(opts.filterable_columns.get(&FieldName::new("title")), Some(&FilterKind::IlikeContains), "title FilterKind survived round-trip",);
    assert_eq!(opts.filterable_columns.get(&FieldName::new("created_at")), Some(&FilterKind::Range),);
    assert_eq!(opts.filterable_columns.get(&FieldName::new("published")), Some(&FilterKind::Bool),);
}
...
fn v2_resource_save_is_byte_stable() {
    ...
    let p1 = dir1.path().join(blast::state::io::RESOURCES_DIR).join("posts.ron");
    let p2 = dir2.path().join(blast::state::io::RESOURCES_DIR).join("posts.ron");
    let h1 = blast::state::hash::content_hash(&p1).expect("hash 1");
    let h2 = blast::state::hash::content_hash(&p2).expect("hash 2");
...
"#;
    ...
    let opts = list_state.list_options.as_ref().expect("list_options present");
    assert_eq!(opts.filterable_columns.len(), 2, "two filterable cols");
    assert_eq!(opts.filterable_columns.get(&FieldName::new("email")), Some(&FilterKind::Eq), "v1 columns default to FilterKind::Eq",);
    assert_eq!(opts.filterable_columns.get(&FieldName::new("id")), Some(&FilterKind::Eq),);
}
...
fn defaults_section_round_trips() {
    ...
    app.sections.insert("defaults".to_string(), AppPolicySection::Defaults(defaults));
    ...
    let section = loaded.sections.get("defaults").expect("defaults section present");
    match section {
        AppPolicySection::Defaults(state) => {
            assert_eq!(state.soft_delete_new_resources_default, Some(SoftDeleteDefault::ExcludeDeleted),);
        }
